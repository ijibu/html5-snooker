...
if (supports_local_storage()) {
	config.skin = getLocalStorage("skin");
	config.player = getLocalStorage("player");
	config.screoboard_pos = getLocalStorage("scoreboard_pos");
	config.screoboard_style = getLocalStorage("scoreboard_style");
	config.shadows = getLocalStorage("shadows");
	config.background = getLocalStorage("background");
	config.hints = getLocalStorage("hints");
	config.cue = getLocalStorage("cue")
}
...
$(function() {
	...
	network.prepare();
	if (getLocalStorage("player") == "Guest") {
	...
	$("#enter-name button[name=save]").click(function() {
		var nick = $("#enter-name input[name=player]").val();
		if (nick != "") {
			setLocalStorage("player", nick);
			setLocalStorage("enter-name", "off");
			$.ajax({
				url: "dashboard",
				cache: false,
				data: {
	...
	$("#news .top-panel").click(function() {
		...
		setLocalStorage("news-" + hex_md5($("#news ul li:first time").text()), "off");
		return false
	});
	...
	function _135() {
		$("#settings input[name=player]").val(getLocalStorage("player"));
		$("#settings select[name=skin]").val(getLocalStorage("skin"));
		$("#settings select[name=scoreboard-style]").val(getLocalStorage("scoreboard_style"));
		$("#settings radio").removeAttr("checked");
		$("#settings input:radio[value=" + getLocalStorage("scoreboard_pos") + "]").attr("checked", "checked");
		$("#settings input[name=sound]").attr("checked", getLocalStorage("sound") == "on");
		$("#settings input[name=shadows]").attr("checked", getLocalStorage("shadows") == "on");
		$("#settings input[name=hints]").attr("checked", getLocalStorage("hints") == "on");
		$("#settings input[name=cue]").attr("checked", getLocalStorage("cue") == "on");
		$("#settings select[name=background]").val(getLocalStorage("background"))
	};
	function _136(init) {
		if (typeof init == "undefined") {
			init = false
		}
		setLocalStorage("player", $("#settings input[name=player]").val());
		setLocalStorage("skin", $("#settings select[name=skin]").val());
		setLocalStorage("scoreboard_pos", $("#settings input:radio[name=scoreboard-position]:checked").val());
		setLocalStorage("scoreboard_style", $("#settings select[name=scoreboard-style]").val());
		setLocalStorage("sound", ($("#settings input[name=sound]").attr("checked") ? "on": "off"));
		setLocalStorage("shadows", ($("#settings input[name=shadows]").attr("checked") ? "on": "off"));
		setLocalStorage("hints", ($("#settings input[name=hints]").attr("checked") ? "on": "off"));
		setLocalStorage("background", $("#settings select[name=background]").val());
		setLocalStorage("cue", ($("#settings input[name=cue]").attr("checked") ? "on": "off"));
		if (!init) {
			$.ajax({
				url: "dashboard",
				cache: false,
				data: {
					"nick": getLocalStorage("player")
				}
			})
		}
		if (typeof globalGame != "undefined") {
			globalGame.shadowsEnable(getLocalStorage("shadows") == "on");
			globalGame.hintsEnable(getLocalStorage("hints") == "on")
		}
		$("link.skin").remove();
		...
		$("head").append($skin);
		scoreboard_pos = getLocalStorage("scoreboard_pos");
		...
		scoreboard_style = getLocalStorage("scoreboard_style");
		...
		$("body").css({
			"background-color": config.backgrounds[getLocalStorage("skin")][getLocalStorage("background")].color,
			"background-image": "url(" + config.backgrounds[getLocalStorage("skin")][getLocalStorage("background")].image + ")"
		})
	};
	_135();
	_136(true);
				...
				if (typeof data != "undefined" && data["servers"].length > 0) {
					...
					function(i, _13e) {
						...
						if (typeof _13e.client_player != "undefined" && _13e.client_player != null && _13e.client_player != "") {
							_13f = "<img src=\"static/media/images/flags/" + _13e.client_lang + ".gif\" class=\"lang\" />" + _13e["client_player"]
						}
						$server = $("<tr />");
						...
						$server.append($("<th />").html("<img src=\"static/media/images/flags/" + _13e["host_lang"] + ".gif\" class=\"lang\" />" + _13e["host_player"]));
	...
	$("button[name=host], a.host").click(function() {
		notify_permission();
		$("input[name=server_name]").val(getLocalStorage("server_name"));
	...
	$("#host-server button[name=start]").click(function() {
		setLocalStorage("server_name", $("input[name=server_name]").val());
		network.ajax("host", {
			"server_name": $("input[name=server_name]").val(),
			"name": getLocalStorage("player"),
